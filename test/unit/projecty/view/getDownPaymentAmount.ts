...
export default function shouldBehaveLikeGetDownPaymentAmount(): void {
  context("when bidId is invalid", function () {
    it("reverts", async function () {
      await expect(this.contracts.projecty.getDownPaymentAmount(78)).to.be.revertedWith(
    ...
    INSTALLMENT_PLAN_VALUES.forEach((installmentPlan) => {
      ...
      context(`when InstallmentPlan.${enumKey}`, () => {
        ...
        it("retrieves correct value", async function () {
          expect(await this.contracts.projecty.getDownPaymentAmount(bidId)).to.be.equal(
            downPayment
          );
        });
      });
