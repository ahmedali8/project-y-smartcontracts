  ...
  context("when the token is not minted", function () {
    ...
    it("reverts", async function () {
      await expect(
        this.contracts.wnft
          .connect(tokenOwner)
          .transferFrom(tokenOwner.address, approved.address, nonExistentTokenId)
    ...
    context("when the sender is not authorized for the token id", function () {
      it("reverts", async function () {
        ...
        await expect(
          this.contracts.wnft
            .connect(other)
            .transferFrom(tokenOwner.address, other.address, tokenId)
    ...
    context("when the address to transfer the token to is the zero address", function () {
      it("reverts", async function () {
        await expect(
          this.contracts.wnft
            .connect(tokenOwner)
            .transferFrom(tokenOwner.address, ZERO_ADDRESS, tokenId)
    ...
    context("when called by the owner", function () {
      it("reverts", async function () {
        await expect(
          this.contracts.wnft
            .connect(tokenOwner)
            .transferFrom(tokenOwner.address, this.signers.accounts[1].address, tokenId)
